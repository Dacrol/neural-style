...
from os import replace as replacef
...
def get_args(arglist=None):
    ...
    parser.add_argument('--wd', type=float, default=.0001,
                        help='weight decay')
    parser.add_argument('--momentum', type=float, default=.95,
                        help='momentum')
...
def train_nstyle(args, callback=None):
    ...
    for e in range(args.max_num_epochs):
        ...
        if (e+1) % args.save_epochs == 0:
            outfn = args.output_dir + 'e_'+str(e+1)+'.png'
        ...
        if callback:
            callback(cbdata)
    try:
        replacef(args.output_dir+'/final.png', args.output_dir+'/previous.png')
    except OSError:
        pass
    final_fn = args.output_dir + '/final.png'
    SaveImage(new_img.asnumpy(), final_fn, args.remove_noise)
